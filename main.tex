...
\date{}
\hypersetup{
    pdftitle={Climatology of North Atlantic Tropical-Cyclone Recurvature Relevant to Newfoundland: Frequency, Location, and Proximity, 1950-2023},
    pdfauthor={Joey Peddle}
}
...
North Atlantic tropical cyclones commonly move westward or northwestward at low latitudes before turning poleward and eastward as the large-scale steering flow changes. North Atlantic track variability is organized partly by genesis region and by the position and strength of large-scale steering features, particularly the subtropical high \citep{Kossin2010,ColbertSoden2012}. Basin activity in the main development and subtropical regions also explains much of the observed interannual variability in the number of recurving North Atlantic cyclones \citep{Sainsbury2022}.

This track turn, conventionally termed recurvature, is distinct from extratropical transition, which is a structural transformation of the cyclone, although the two often occur during overlapping portions of a cyclone's movement into the baroclinic mid-latitudes \citep{HartEvans2001,EvansHart2003,Jones2003}. Transitioning cyclones can retain or redevelop substantial wind, rainfall, and wave hazards and can modify the downstream mid-latitude flow \citep{Evans2017,Keller2019}. Newfoundland can therefore experience effects from a recurving cyclone of tropical origin even when the cyclone centre remains offshore.

Recurvature is not a uniquely defined event. Published work has used distinct operational classifications; for example, \citet{Sainsbury2022} classified North Atlantic cyclones as recurving when they entered a broad mid-latitude domain, without requiring a change in zonal direction. A detector based only on longitude increments can instead confuse eastward acceleration with an actual turn, while a detector applied to irregular track intervals assigns different physical meanings to identical numerical thresholds. Regional relevance adds a second definitional problem: landfall-only classifications exclude offshore systems, whereas broad distance thresholds can include pathways with little direct impact potential. These choices must therefore be expressed in physical units, tested explicitly, and kept separate from claims about observed damage or risk.
...
The primary estimand is the log-linear trend in annual baseline event count over 1950--2023. Annual counts were modelled using Poisson regression with a log link and centred year predictor. Effect magnitude is reported as an incidence-rate ratio (IRR) per decade with a 95\% confidence interval. Pearson \(\chi^2/\mathrm{df}\) was used to assess dispersion. As a dependence check, confidence intervals were also calculated with heteroskedasticity-and-autocorrelation-consistent covariance using two lags \citep{NeweyWest1987}; Pearson-residual lag-1 correlation and a Ljung--Box test \citep{LjungBox1978} were inspected.

Two binomial models distinguish pathway behaviour from raw occurrence. The first models Newfoundland-relevant events among eligible storms. The second models Newfoundland-relevant events among detected recurvers. Both report an odds ratio (OR) per decade. Analyses were performed for 1950--2023 and for 1979--2023, with 1979 used as a fixed, coarse modern satellite-era boundary rather than an estimated changepoint. This boundary is a sensitivity period, not a claim that Atlantic best-track observations became homogeneous in a single year \citep{VecchiKnutson2008,LandseaFranklin2013}. Detector and proximity sensitivities were evaluated for both periods.

Linear trends in recurvature latitude and longitude were estimated using ordinary least squares with HC3 covariance \citep{MacKinnonWhite1985}. For the joint early- versus late-era comparison, longitude and latitude were transformed to three-dimensional Earth-centred Cartesian coordinates in kilometres, and a two-sample energy-distance test was performed with 4999 permutations \citep{SzekelyRizzo2013}. Minimum-distance trends were estimated for all detected recurvers using both HC3 mean regression and median quantile regression \citep{KoenkerBassett1978}. Results conditional on the 600-km selection were retained as secondary diagnostics. As a censoring diagnostic, both distance trends were repeated after excluding tracks whose minimum occurred at the final recorded point.
...
\input{outputs/table_method_comparison.tex}
...
\input{outputs/table_frequency_trends.tex}
...
\input{outputs/table_pathway_trends.tex}
...
\input{outputs/table_location_proximity.tex}
\input{outputs/table_endpoint_sensitivity.tex}
...
\input{outputs/table_threshold_sensitivity.tex}
\input{outputs/table_satellite_threshold_sensitivity.tex}
...
Earlier best-track data are less homogeneous than the satellite-era record, with historical observing changes affecting the completeness and uncertainty of Atlantic tropical-cyclone records \citep{VecchiKnutson2008,LandseaFranklin2013}. Linear interpolation cannot recover unobserved track detail, Natural Earth geometry introduces finite coastline resolution, and projected distances are not identical across coordinate systems. The projection sensitivity indicates that these geometric choices affect a few boundary classifications but not the trend interpretation. The 500-hPa composite includes the cyclone circulation, only one pressure level, and no matched control population; it cannot establish Newfoundland-specific dynamics. Finally, linear models summarize monotonic change but do not fully describe decadal clustering or nonlinear variability.

\subsection{Future work}
Future work should connect track proximity to intensity, storm size, extratropical-transition phase, and verified Newfoundland impacts. Deep-layer steering-flow and potential-vorticity composites, which are central to diagnosing tropical-cyclone interaction with the mid-latitude circulation \citep{Jones2003,Keller2019}, should be compared with matched non-Newfoundland recurvers. Track uncertainty could be propagated through the detector using perturbed-track ensembles. A separate coastline-intersection analysis would also distinguish landfalls from offshore hazard pathways without conflating the two.
...
\textit{Journal of the Royal Statistical Society: Series B (Methodological)}, \textit{57}(1), 289--300.
\url{https://doi.org/10.1111/j.2517-6161.1995.tb02031.x}

\bibitem[Colbert and Soden(2012)]{ColbertSoden2012}
Colbert, A.~J., and Soden, B.~J. (2012).
Climatological variations in North Atlantic tropical cyclone tracks.
\textit{Journal of Climate}, \textit{25}(2), 657--673.
\url{https://doi.org/10.1175/JCLI-D-11-00034.1}

\bibitem[Evans et~al.(2017)]{Evans2017}
Evans, C., Wood, K.~M., Aberson, S.~D., Archambault, H.~M., Milrad, S.~M., Bosart, L.~F., Corbosiero, K.~L., Davis, C.~A., and coauthors (2017).
...
\url{https://doi.org/10.1175/MWR-D-17-0027.1}
...
\textit{Monthly Weather Review}, \textit{131}(5), 909--925.
\url{https://doi.org/10.1175/1520-0493(2003)131<0909:OIOTLC>2.0.CO;2}

\bibitem[Gahtan et~al.(2024)]{Gahtan2024}
Gahtan, J., Knapp, K.~R., Schreck, C.~J., III, Diamond, H.~J., Kossin, J.~P., and Kruk, M.~C. (2024).
International Best Track Archive for Climate Stewardship (IBTrACS) Project, Version 4.
North Atlantic basin subset, release v04r00, 1950--2023.
NOAA National Centers for Environmental Information.
Accessed 25 July 2026.
\url{https://doi.org/10.25921/82ty-9e16}

\bibitem[Hart and Evans(2001)]{HartEvans2001}
Hart, R.~E., and Evans, J.~L. (2001).
A climatology of the extratropical transition of Atlantic tropical cyclones.
\textit{Journal of Climate}, \textit{14}(4), 546--564.
\url{https://doi.org/10.1175/1520-0442(2001)014<0546:ACOTET>2.0.CO;2}

\bibitem[Jones et~al.(2003)]{Jones2003}
Jones, S.~C., Harr, P.~A., Abraham, J., Bosart, L.~F., Bowyer, P.~J., Evans, J.~L., Hanley, D.~E., and coauthors (2003).
...
\url{https://doi.org/10.1175/1520-0434(2003)018<1052:TETOTC>2.0.CO;2}

\bibitem[Kalnay et~al.(1996)]{Kalnay1996}
Kalnay, E., Kanamitsu, M., Kistler, R., Collins, W., Deaven, D., Gandin, L., Iredell, M., Saha, S., White, G., Woollen, J., Zhu, Y., and coauthors (1996).
The NCEP/NCAR 40-Year Reanalysis Project.
\textit{Bulletin of the American Meteorological Society}, \textit{77}(3), 437--471.
\url{https://doi.org/10.1175/1520-0477(1996)077<0437:TNYRP>2.0.CO;2}

\bibitem[Keller et~al.(2019)]{Keller2019}
Keller, J.~H., Grams, C.~M., Riemer, M., Archambault, H.~M., Bosart, L.~F., Doyle, J.~D., Evans, J.~L., Galarneau, T.~J., and coauthors (2019).
...
\textit{Monthly Weather Review}, \textit{147}(4), 1077--1106.
\url{https://doi.org/10.1175/MWR-D-17-0329.1}

\bibitem[Kelso and Patterson(2009)]{KelsoPatterson2009}
Kelso, N.~V., and Patterson, T. (2009).
Natural Earth Vector.
\textit{Cartographic Perspectives}, \textit{64}, 45--50.
\url{https://doi.org/10.14714/CP64.148}
...
\textit{Bulletin of the American Meteorological Society}, \textit{91}(3), 363--376.
...
\bibitem[Kossin et~al.(2010)]{Kossin2010}
Kossin, J.~P., Camargo, S.~J., and Sitkowski, M. (2010).
Climate modulation of North Atlantic hurricane tracks.
\textit{Journal of Climate}, \textit{23}(11), 3057--3076.
\url{https://doi.org/10.1175/2010JCLI3497.1}
...
\bibitem[Sainsbury et~al.(2022)]{Sainsbury2022}
Sainsbury, E.~M., Schiemann, R.~K.~H., Hodges, K.~I., Baker, A.~J., Shaffrey, L.~C., and Bhatia, K.~T. (2022).
What governs the interannual variability of recurving North Atlantic tropical cyclones?
\textit{Journal of Climate}, \textit{35}(12), 3627--3641.
\url{https://doi.org/10.1175/JCLI-D-21-0712.1}

\bibitem[Sz\'ekely and Rizzo(2013)]{SzekelyRizzo2013}
Sz\'ekely, G.~J., and Rizzo, M.~L. (2013).
Energy statistics: A class of statistics based on distances.
\textit{Journal of Statistical Planning and Inference}, \textit{143}(8), 1249--1272.
\url{https://doi.org/10.1016/j.jspi.2013.03.018}

\bibitem[Vecchi and Knutson(2008)]{VecchiKnutson2008}
Vecchi, G.~A., and Knutson, T.~R. (2008).
On estimates of historical North Atlantic tropical cyclone activity.
\textit{Journal of Climate}, \textit{21}(14), 3580--3600.
\url{https://doi.org/10.1175/2008JCLI2178.1}
